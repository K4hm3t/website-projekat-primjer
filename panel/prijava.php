...
if (isset($_POST['korisnik'])) {    // ako je unesen username, ako je ispunjen uslov if statmenta izvrsava se kod ispod
	...
	if (mysqli_num_rows($rezultat) == 1) {												// putem funkcije mysqli_query
		...
		echo'<script>window.location="panel/index.php";</script>'; return true;			// unutar zagrade a i spremanjem u varijablu kao u ovom slucaju
	} else {																			// zatim pozivanjem iste
		echo "<script>alert('Netačan username ili šifra.')</script>"; return true;		// uslov u if statmentu jeste mysqlu_num_rows za prethodni upit
